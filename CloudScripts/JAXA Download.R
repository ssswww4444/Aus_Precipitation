...
JAXA = list()
start1 = length(JAXA.Monthly) + 1
for(i in start1:nrow(dates4)){
  file1 = "ftp://swcem:SEMaP+2004@hokusai.eorc.jaxa.jp/EAWP/GSMaP_GNRT/DATA/"
  file2 = dates4[i,1]
  file3 = dates4[i,2]
  if(nchar(file3) == 1){
...
for(i in start1:length(JAXA)){
  ...
  write.csv(JAXA[[i]][samp1,], file = paste0(file1, file2, file3), row.names = FALSE)
}
...
JAXA.daily = list()
pb <- txtProgressBar(min = 134, max = nrow(dates4), style = 3)
start1 = 1
...
Daily.Dates2.2 = rep(0, sum(dates4[130:261,3]))
k = 1
for(i in 130:261){
  for(j in 1:dates4[i,3]){
    mon1 = dates4[i,2]
...
ggplot() + geom_polygon(data = Australasiamap, aes(x = long, y = lat, group = group), colour = "black", fill = NA) +
  ...
  geom_point(data = Diff2.df, 
             mapping = aes(x = Lon, y = Lat, colour = Diff), size = 0.1)+
  coord_cartesian(xlim = c(115,178), ylim = c(-50, 0)) +
  scale_colour_manual(
  ...
  labs(color = "Estimate", x = "Longitude", y = "Latitude",
       title = "Significant Coefficient Differences") +  theme_bw() + 
  theme(plot.title = element_text(size = 18, face = "bold"),
        legend.title=element_text(size=18), axis.text=element_text(size=12), legend.text=element_text(size=13),
...
pred.grid = data.frame(Longitude = coordinates(s1)[,1], Latitude = coordinates(s1)[,2])
idw1 = idw(formula = Amplitude ~ 1, locations = ~Longitude + Latitude, data = Coefs.df, newdata = pred.grid, idp = 3)
idw2 = idw(formula = Period ~ 1, locations = ~Longitude + Latitude, data = Coefs.df, newdata = pred.grid, idp = 3)
idw3 = idw(formula = Shift ~ 1, locations = ~Longitude + Latitude, data = Coefs.df, newdata = pred.grid, idp = 3)
...
A.plot <- ggplot() + geom_polygon(data = Australasiamap, aes(x = long, y = lat, group = group), colour = "black", fill = NA) +
  ...
  theme(plot.title = element_text(size = 18, face = "bold"),
        legend.title=element_text(size=18), axis.text=element_text(size=12), legend.text=element_text(size=13),
        legend.key.size = unit(1, "cm"), axis.title=element_text(size=15), legend.position = "right")
...
grid.arrange(A.plot + labs(x = " ", y = " ", colour = " ") + theme(legend.position = "bottom"), P.plot + labs(x = " ", y = " ", colour = " ") + theme(legend.position = "bottom", axis.text.y=element_blank()),
             ...
             nrow = 1, left = textGrob("Latitude", gp=gpar(fontsize=16,font=8), rot = 90, hjust = -0.5),
             bottom = textGrob("Longitude", gp=gpar(fontsize=16,font=8), vjust = -8),
...
ggplot(data = d2, aes(x = Dates, y = Count)) + geom_point() + facet_wrap(~ Loc) + scale_x_date(date_labels = "%b", date_breaks = "2 month")+
  theme_bw() + labs(y = "Proportion", x = "Date", title = "Proportion of Rain Days")+ 
  theme(plot.title = element_text(size = 20, face = "bold"),
        legend.title=element_text(size=20), axis.text=element_text(size=14), legend.text=element_text(size=15),
        legend.key.size = unit(1, "cm"), axis.title=element_text(size=16), legend.position = "right", strip.text = element_text(size=16))
...
years2 = NULL
for(i in 1:length(unique(years1))){
  years2 = c(years2, rep(unique(years1)[i], sum(dates4[dates4[,1] == unique(years1)[i],3])))
}

years2 = years2[1:ncol(JAXA.all)]

JAXA.any = JAXA.all > 0

JAXA.year = matrix(0, nrow(JAXA.any), ncol = length(unique(years2)))
for(i in 1:length(unique(years2))){
  JAXA.year[,i] = rowSums(JAXA.any[,which(years2 == unique(years2)[i])])
}

JAXA.year1 = JAXA.year[,2:(ncol(JAXA.year))]


MeanFreqdf = data.frame(Lon = JAXA.daily1[[1]]$longitude, Lat = JAXA.daily1[[1]]$Latitude, Mean = rowMeans(JAXA.year1))

set.seed(1998)
s1 = spsample(Australasiamap,n=200000,"random")

pred.grid = data.frame(Lon = coordinates(s1)[,1], Lat = coordinates(s1)[,2])
idw1 = idw(formula = Mean ~ 1, locations = ~Lon + Lat, data = MeanFreqdf, newdata = pred.grid, idp = 3)


ggplot() + geom_polygon(data = ausmap, aes(x = long, y = lat, group = group), colour = "black", fill = NA) + 
  geom_point(idw1, mapping = aes(x = Lon, y = Lat, colour = var1.pred), size = 0.1) +
  theme_bw() + labs(x = "Longitude", y = "Latitude", title = "Mean Annual Precipitation Days", colour = "Number\nof Days")+ 
  theme(plot.title = element_text(size = 18, face = "bold"),
        legend.title=element_text(size=18), axis.text=element_text(size=12), legend.text=element_text(size=13),
        legend.key.size = unit(1, "cm"), axis.title=element_text(size=14), legend.position = "right") +
  scale_colour_gradientn(colours=c("#00007F", "blue", "#007FFF", "cyan",
...
Freq.coef = matrix(0, nrow = nrow(JAXA.year1), ncol = 4)
for(i in 1:nrow(JAXA.year1)){
  y = JAXA.year1[i,]
  x = 2001:2021
  mod1 = lm(y ~ x)
  Freq.coef[i,] = c(mod1$coefficients, summary(mod1)$coefficients[2,4],summary(mod1)$s)
}


colnames(Freq.coef) = c("Intercept", "Trend", "Pvalue", "Sigma")
Freq.coef = as.data.frame(Freq.coef)

Freq.coef$Lon = JAXA.daily1[[1]]$longitude
Freq.coef$Lat = JAXA.daily1[[1]]$Latitude

pred.grid = data.frame(Lon = coordinates(s1)[,1], Lat = coordinates(s1)[,2])
idw1 = idw(formula = Pvalue ~ 1, locations = ~Lon + Lat, data = Freq.coef, newdata = pred.grid, idp = 3)

idw1$z = idw1$var1.pred < 0.05

idw2 = idw1[idw1$var1.pred < 0.05,]

ggplot() + geom_polygon(data = Australasiamap, aes(x = long, y = lat, group = group), colour = "black", fill = NA) + 
  geom_point(idw2, mapping = aes(x = Lon, y = Lat), colour = "red", size = 0.1) +
  theme_bw() + labs(x = "Longitude", y = "Latitude", title = "Significant Annual Frequency Trends")+ 
  theme(plot.title = element_text(size = 18, face = "bold"),
        legend.title=element_text(size=18), axis.text=element_text(size=12), legend.text=element_text(size=13),
        legend.key.size = unit(1, "cm"), axis.title=element_text(size=14), legend.position = "right") +
  coord_cartesian(xlim = c(115,180), ylim = c(-50, 0))
...
JAXA.seas = list()
for(i in 1:nrow(JAXA.daily1[[1]])){
  JAXA.seas[[i]] = list()
  for(j in 1:12){
    Temp = NULL
    for(k in D4[[j]]){
      if(j == 2){
        Temp = rbind(Temp, JAXA.daily1[[k]][i,1:30])
      }else{
        Temp = rbind(Temp, JAXA.daily1[[k]][i,])
      }
    }
    JAXA.seas[[i]][[j]] = Temp
  }
...
save(JAXA.seas, file = "JAXA.seas")

load("JAXA.seas")

Seas.per = matrix(0, nrow = nrow(JAXA.daily1[[1]]), ncol = 14)
for(i in 1:nrow(JAXA.daily1[[1]])){
  for(j in 1:12){
    Seas.per[i, j + 2] = quantile(as.vector(as.matrix(JAXA.seas[[i]][[j]][,-c(1,2)])), 0.99)
  }
}

Seas.per[,1] = JAXA.daily1[[1]]$longitude
Seas.per[,2] = JAXA.daily1[[1]]$Latitude


colnames(Seas.per) = c("Lon", "Lat", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
Seas.per = as.data.frame(Seas.per)


D3 = rep(dates4[,2], dates4[,3])
D3 = D3[1:ncol(JAXA.all)]
JAXA.99 = matrix(0, nrow = nrow(JAXA.all), ncol = ncol(JAXA.all))
for(i in 1:12){
  for(j in 1:ncol(JAXA.all)){
    JAXA.99[j, which(D3 == i)] = (JAXA.all[j, which(D3 == i)] > Seas.per[j, i + 2])
  }
}
...
J1 = JAXA.99[c(Port.Moresby, Canberra, Wellington),]
...
for(i in 1:nrow(dates4)){
  ...
  for(j in 1:dates4[i,3]){
    ...
    JAXA.dates = c(JAXA.dates, paste0(dates4[i,1], "-", m1, "-", m2))
  }
}

JAXA.dates = JAXA.dates[1:ncol(JAXA.99)]
JAXA.dates = as.Date(JAXA.dates)

J1 = data.frame(Rain = c(JAXA.99[Port.Moresby,], JAXA.99[Canberra,], JAXA.99[Wellington,]),
                Location = factor(rep(c("Port Moresby", "Canberra", "Wellington"), each = ncol(JAXA.99)), levels = c("Port Moresby", "Canberra", "Wellington")),
                Dates = rep(JAXA.dates, 3))


ggplot() + 
  geom_point(J1, mapping = aes(x = Dates, y = Rain), size = 0.1) + facet_wrap(~Location, nrow = 3) + 
  theme_bw() + labs(x = "Longitude", y = "Latitude", title = "Mean Annual Precipitation Days", colour = "Number\nof Days")+ 
  theme(plot.title = element_text(size = 18, face = "bold"),
        legend.title=element_text(size=18), axis.text=element_text(size=12), legend.text=element_text(size=13),
        legend.key.size = unit(1, "cm"), axis.title=element_text(size=14), legend.position = "right") +
...
quantile(as.vector(as.matrix(JAXA.seas[[Port.Moresby]][[1]][,-c(1,2)])), 0.99)
