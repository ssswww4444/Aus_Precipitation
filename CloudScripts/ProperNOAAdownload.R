...
download.file(file1, destfile = "NOAA.nc1", method = "libcurl")

ncin = nc_open("NOAA.nc1")
...
setwd("/nfs/ms_home/home/ad/student.unimelb.edu.au/bhines/NOAAData")
NOAA.aus = list()
for(i in 1:nrow(datesNOAA)){
  ...
  NOAA.aus[[i]] = read.csv(paste0("NOAA.aus", datesNOAA[i,1], m1))
}
...
NOAA = list()
k = 43 * 12 + 1
for(i in start1:length(NOAA.mon)){
  for(j in 1:12){
...
start2 = 43 * 12 + 1
for(i in start2:length(NOAA)){
  NOAA[[i]] = cbind(NOAA[[i]], rep(datesNOAA[i,2], nrow(NOAA[[length(NOAA)]])), rep(datesNOAA[i,1], nrow(NOAA[[length(NOAA)]])))
  colnames(NOAA[[i]]) = c("Lat", "Lon", "Precipitation", "Month", "Year")
  NOAA[[i]] = as.data.frame(NOAA[[i]])
}

a1 = rep(TRUE, nrow(NOAA[[length(NOAA)]]))
a2 = Inf
a3 = 1
a4 = NULL
...
for(i in (43 * 12 + 1):nrow(datesNOAA)){
  if(a4[i] < nrow(NOAA[[i]])){
...
for(i in (43 * 12 + 1):nrow(datesNOAA)){
  ...
  write.csv(NOAA.aus[[i]], paste0("NOAA.aus", datesNOAA[i,1], m2), row.names = FALSE)
}



g.cor = ggplot() + geom_polygon(data = shape, aes(x = long, y = lat, group = group), colour = "black", fill = NA) +
  geom_point(data = NOAA[[(43 * 12 + 1)]], 
...
g.cor1 = ggplot() + geom_polygon(data = ausmap, aes(x = long, y = lat, group = group), colour = "black", fill = NA) +
  geom_point(data = NOAA.aus[[(43 * 12 + 1)]], 
             mapping = aes(x = Lon, y = Lat, colour = Precipitation), size = 0.1) +
  scale_colour_gradientn(colours=c("#00007F", "blue", "#007FFF", "cyan",
                                   "#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000"), values = seq(1,0, length.out = 9)) + theme_bw() +
  theme(plot.title = element_text(size = 15, face = "bold"),
...
for(i in 1:nrow(datesNOAA)){
  ...
  NOAA.aus[[i]] = read.csv(paste0("NOAA.aus", datesNOAA[i,1], m2))
}
