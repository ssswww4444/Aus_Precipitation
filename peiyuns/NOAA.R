...
# tmp array with size lon*lat*days
dname = "precip"
tmp.array <- ncvar_get(ncin, dname)

# check calendar (number of days in each year and month for summing)
...
# NOAA.month stores monthly data for each year, each element is a year
NOAA.mon = list()
...
for(i in 1:length(years)){
  ...
  # store in corresponding WORLD dataframe
  if(nt == 365){
    for(j in 1:nt){
...
# NOAA store monthly data, each element is a month
NOAA = list()
k = 1
...
# Add location and date to NOAA, convert into dataframe
...
for(i in 1:length(NOAA)){
  NOAA[[i]][,1] = ifelse(NOAA[[i]][,1] > 90, NOAA[[i]][,1] - 180, NOAA[[i]][,1]) # lat
}
